...
///
struct IMPLPersistentOrderedMapTree<K,V> where K:Comparable {
    let rootKey:K
...
extension IMPLPersistentOrderedMapTree {
    ...
    mutating func replaceSubtrees<C>(_ r:Range<Int>, in pk:K, with ts:C) where C:Collection, C.Element == IMPLPersistentOrderedMapTree {
    ...
    mutating func replaceSubrange<C>(_ r:Range<Int>, in pk:K, with kvs:C) where C:Collection, C.Element == (K,V) {
        let ts1 = kvs.map({ IMPLPersistentOrderedMapTree(root: $0) })
    ...
    private mutating func insertSubtrees<C>(_ ts:C, at i:Int, in pk:K) where C:Collection, C.Element == IMPLPersistentOrderedMapTree {
        for t in ts {
