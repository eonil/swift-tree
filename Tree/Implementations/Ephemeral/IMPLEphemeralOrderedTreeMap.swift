//
//  IMPLEphemeralOrderedTreeMap.swift
//  Tree
//
...
/// Internal implementation of ephemeral and explicitly ordered tree-map.
///
...
///
struct IMPLEphemeralOrderedTreeMap<K,V> where K:Hashable {
    let rootKey:K
...
extension IMPLEphemeralOrderedTreeMap {
    typealias StateMap = Dictionary<K,V>
    typealias LinkageMap = Dictionary<K,Children>
    typealias Children = Array<K>
}
...
extension IMPLEphemeralOrderedTreeMap {
    ...
    mutating func replaceSubtrees<C>(_ r:Range<Int>, in pk:K, with ts:C) where C:Collection, C.Element == IMPLEphemeralOrderedTreeMap {
    ...
    mutating func replaceSubrange<C>(_ r:Range<Int>, in pk:K, with kvs:C) where C:Collection, C.Element == (K,V) {
        let ts1 = kvs.map({ IMPLEphemeralOrderedTreeMap(root: $0) })
    ...
    private mutating func insertSubtrees<C>(_ ts:C, at i:Int, in pk:K) where C:Collection, C.Element == IMPLEphemeralOrderedTreeMap {
        for t in ts {
